...
class K2Header {
	function init() {
		$columns = get_option('unwakeable_columns');
	...
	function install() {
		add_option('unwakeable_headerimage', '', 'Current Header Image');
		add_option('unwakeable_blogornoblog', 'Blog', 'The text on the first tab in the header navigation.');
	}
	...
	function display_options() {
		...
		$current_header_image = get_option('unwakeable_headerimage');
...
<?php
	...
	function update_options() {
		...
		if ( isset($_POST['k2']['blogornoblog']) ) {
			update_option( 'unwakeable_blogornoblog', strip_tags( stripslashes($_POST['k2']['blogornoblog']) ) );
		}

		// Header Image
		if ( isset($_POST['k2']['headerimage']) ) {
			update_option('unwakeable_headerimage', $_POST['k2']['headerimage']);
	...
	function get_header_image_url() {
		$header_image = get_option('unwakeable_headerimage');
	...
	function process_custom_header_image($source, $id = 0) {
		...
		if ( file_exists($source) and (strpos(basename($source),'midsize-') === false) ) {
			...
			// Update K2 Options
			update_option( 'unwakeable_headerimage', $id );
		}
