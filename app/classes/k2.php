...
class K2 {
	 ...
	 */
	function install() {
		add_option('unwakeable_version', K2_CURRENT, 'This option stores K2\'s version number');
		add_option('unwakeable_asidescategory', '0', 'A category which will be treated differently from other categories');
		add_option('unwakeable_livesearch', '1', "If you don't trust JavaScript and Ajax, you can turn off LiveSearch. Otherwise I suggest you leave it on"); // (live & classic)
		add_option('unwakeable_rollingarchives', '1', "If you don't trust JavaScript and Ajax, you can turn off Rolling Archives. Otherwise it is suggested you leave it on");
		add_option('unwakeable_archives', '0', 'Set whether K2 has an archives page');
		add_option('unwakeable_columns', '2', 'Number of columns to display.');
		...
		add_option('unwakeable_ajaxdonejs', $defaultjs, 'JavaScript to execute when Ajax is completed');
	...
	function upgrade($previous) {
		...
		// Update the version
		update_option('unwakeable_version', K2_CURRENT);
	}
	...
	function uninstall() {
		// Delete options
		delete_option('unwakeable_version');
		delete_option('unwakeable_asidescategory');
		delete_option('unwakeable_livesearch');
		delete_option('unwakeable_rollingarchives');
		delete_option('unwakeable_archives');
		delete_option('unwakeable_columns');
		delete_option('unwakeable_entrymeta1');
		delete_option('unwakeable_entrymeta2');
		delete_option('unwakeable_animations');
		delete_option('unwakeable_ajaxdonejs');
	...
	function enqueue_scripts() {
		...
		if ( ! is_admin() ) {
			...
			if ( '1' == get_option('unwakeable_rollingarchives') )
				wp_enqueue_script('unwakeable_rollingarchives');

			if ( '1' == get_option('unwakeable_livesearch') )
				wp_enqueue_script('unwakeable_livesearch');
...
?>
	<script type="text/javascript">
	//<![CDATA[
	<?php if ( 'dynamic' == get_option('unwakeable_columns') ): ?>
	...
	<?php endif; ?>
		...
		jQuery(document).ready(function(){
			...
			jQuery('#dynamic-content').ajaxSuccess(function () {
				<?php echo get_option('unwakeable_ajaxdonejs'); ?>
			});
