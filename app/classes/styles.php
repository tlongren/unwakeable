...
class unwakeable_styles {
	/**
	 * Initializes unwakeable_styles
	...
	function init() {
		...
		if ( ! empty($active_styles) ) {
			foreach ($active_styles as $style) {
				$style_functions = dirname( unwakeable_styles::get_styles_dir() . '/' . $style ) . '/functions.php';
	 ...
	 */
	function install() {
		add_option('unwakeable_styles', array(), 'Choose the Style you want K2 to use');
		add_option('unwakeable_styleinfo', '', 'Metadata of current style.');
		add_option('unwakeable_stylespath', '%k2%/styles', 'Location of K2 Styles');
		add_option('unwakeable_stylesdir', TEMPLATEPATH . '/styles', 'Directory of K2 Styles - Autogenerated');
		add_option('unwakeable_stylesurl', TEMPLATEPATH . '/styles', 'URL of K2 Styles - Autogenerated');
	}
	 ...
	 */
	function uninstall() {
		delete_option('unwakeable_styles');
		delete_option('unwakeable_styleinfo');
		delete_option('unwakeable_stylespath');
		delete_option('unwakeable_stylesdir');
		delete_option('unwakeable_stylesurl');
	}
	...
	function upgrade($previous) {
		if ( version_compare($previous, '1.0-RC8', '<') ) {
			...
			if ( ! empty($style) ) {
				update_option( 'unwakeable_styles', array($style) );
			}
		}
	}
	 ...
	 *
	 * @param string $path unwakeable_stylespath
	 * @return string absolute path
	 */
	function set_styles_dir( $path = false ) {
		if ( empty($path) )
			$path = get_option('unwakeable_stylespath');
		...
		update_option( 'unwakeable_stylesdir', $dir );
	...
	function get_styles_dir( $path = false ) {
		$dir = get_option('unwakeable_stylesdir');

		if ( ! empty($dir) )
			return $dir;

		return unwakeable_styles::set_styles_dir( $path );
	}
	...
	function set_styles_url( $path = false ) {
		...
		update_option( 'unwakeable_stylesurl', $url );
	...
	function get_styles_url( $path = false ) {
		$url = get_option('unwakeable_stylesurl');

		if ( ! empty($url) )
			return $url;

		return unwakeable_styles::set_styles_url( $path );
	}
	...
	function display_options() {
		$styles_path = explode('/', get_option('unwakeable_stylespath'), 2);
		$styles_dir = get_option('unwakeable_stylesdir');

		// Get the current K2 Style
		$active_styles = (array) get_option('unwakeable_styles');

		// Get the style files
		$style_files = unwakeable_styles::get_styles();
...
<?php
	...
	function update_options() {
		...
		if ( isset($_POST['k2']['styles']) ) {
			update_option('unwakeable_styles', $_POST['k2']['styles']);
			unwakeable_styles::update_style_info();
		} else {
			update_option('unwakeable_styles', array());
			update_option('unwakeable_styleinfo', array());
		}
		...
		if ( isset($_POST['k2']['stylesroot']) and isset($_POST['k2']['stylesdir']) ) {
			...
			update_option( 'unwakeable_stylespath', $path );
			unwakeable_styles::set_styles_dir( $path );
			unwakeable_styles::set_styles_url( $path );
		}
	}
	...
	function get_styles() {
		...
		$style_files = K2::files_scan( unwakeable_styles::get_styles_dir(), 'css', 2 );
		sort($style_files);
		...
		foreach ( (array) $style_files as $style_file ) {
			$style_data = unwakeable_styles::get_style_data($style_file);
	...
	function theme_editor_append_styles() {
		...
		$styles_dir = unwakeable_styles::get_styles_dir();
	...
	function update_style_info() {
		$data = unwakeable_styles::get_style_data( array_shift( get_option('unwakeable_styles') ) );
		...
		update_option('unwakeable_styleinfo', $data);
	...
	function get_style_data( $style_file = '' ) {
		...
		$style_path = unwakeable_styles::get_styles_dir() . "/$style_file";
...
add_action( 'k2_init', array('unwakeable_styles', 'init') );
add_action( 'k2_install', array('unwakeable_styles', 'install') );
add_action( 'k2_uninstall', array('unwakeable_styles', 'uninstall') );

add_action( 'k2_display_options', array('unwakeable_styles', 'display_options'), 15 );
add_action( 'k2_update_options', array('unwakeable_styles', 'update_options') );

add_action( 'admin_init', array( 'unwakeable_styles', 'theme_editor_append_styles') );
add_action( 'wp_head', array('unwakeable_styles', 'load_styles') );