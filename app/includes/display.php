...
?>
	...
	<?php else: ?>
		...
		<div class="nav-previous"><?php next_posts_link( '<span class="meta-nav">&laquo;</span> ' . __('Older','unwakeable') ); ?></div>
		<div class="nav-next"><?php previous_posts_link( __('Newer','unwakeable').' <span class="meta-nav">&raquo;</span>' ); ?></div>
	<?php endif; ?>
...
function k2_permalink_title($echo = true) {
	$output = sprintf( __('Permanent Link to %s','unwakeable'), esc_html( strip_tags( the_title('', '', false) ), 1) );
...
function k2_nice_category($normal_separator = ', ', $penultimate_separator = ' and ') { 
	...
	if (empty($categories)) { 
		return __('Uncategorized','unwakeable');
	} 
