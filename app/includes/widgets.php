...
class K2_Widget_Asides extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		$unwakeable_asidescategory = get_option('unwakeable_asidescategory');

		if ( $unwakeable_asidescategory != '0') {
			$title = empty($instance['title']) ? apply_filters('single_cat_title', get_the_category_by_ID($unwakeable_asidescategory)) : apply_filters('widget_title', $instance['title']);

			$asides = new WP_Query( array( 'cat' => $unwakeable_asidescategory, 'showposts' => $instance['number'], 'what_to_show' => 'posts', 'nopaging' => 0, 'post_status' => 'publish', 'caller_get_posts' => 1 ) );
...
function k2_asides_filter($query) {
	$asidescat = get_option('unwakeable_asidescategory');
